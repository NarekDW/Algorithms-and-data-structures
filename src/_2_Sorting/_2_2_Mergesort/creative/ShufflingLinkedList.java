...
public class ShufflingLinkedList {
    ...
    public static <T extends Comparable<T>> void shuffle(DoubleLinked<T> linked) {
        int n = linked.size();
    ...
    private static <T extends Comparable<T>> void shuffle(DoubleLinked<T>.DoubleNode[] aux,
                                                          int point1,
                                                          int point2,
                                                          int offset1,
                                                          int offset2) {
        ...
        if (a1 == a2Prev) {
            swapNeighborhoods(a1, a2);
        } else if (a2 == a1Prev) {
            swapNeighborhoods(a2, a1);
        } else {
            swap(a1, a2, a1Prev, a1Next, a2Prev, a2Next);
        }
    }

    private static <T extends Comparable<T>> void swapNeighborhoods(DoubleLinked<T>.DoubleNode a1,
                                                                    DoubleLinked<T>.DoubleNode a2) {
        DoubleLinked<T>.DoubleNode a1Prev = a1.prev;
        a1.next = a2.next;
        if (a1.next != null)
            a1.next.prev = a1;
    ...
    private static <T extends Comparable<T>> void swap(DoubleLinked<T>.DoubleNode a1,
                                                       DoubleLinked<T>.DoubleNode a2,
                                                       DoubleLinked<T>.DoubleNode a1Prev,
                                                       DoubleLinked<T>.DoubleNode a1Next,
                                                       DoubleLinked<T>.DoubleNode a2Prev,
                                                       DoubleLinked<T>.DoubleNode a2Next) {
        if (a1Prev != null)
            a1Prev.next = a2;
        a2.prev = a1Prev;
    ...
    private static void checkContent(int size, DoubleLinked<Integer> linked) {
        for (int i = 0; i < size; i++)
            if (!linked.contains(i + 1))
                throw new RuntimeException(String.valueOf(i + 1));
    }
